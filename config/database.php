    ...
    */
    ...
    'connections' => [
        ...
        'sqlite' => [
            ...
            'database' => (function() {
                $db = env('DB_DATABASE');
                if ($db && (str_ends_with($db, '.sqlite') || str_ends_with($db, '.db') || str_contains($db, '/'))) {
                    return $db;
                }
        ...
        'mysql' => [
            'driver' => 'mysql',
            'url' => env('DB_URL') ?: (env('MYSQL_URL') ?: (env('MYSQL_PRIVATE_URL') ?: env('DATABASE_URL'))),
            'host' => (function() {
            ...
            'port' => (function() {
                ...
                $dbPort = env('DB_PORT');
                return ($dbPort && $dbPort !== '3306') ? $dbPort : (env('MYSQLPORT') ?: (env('MYSQL_PORT') ?: ($dbPort ?: '3306')));
            })(),
            'database' => (function() {
                if ($url = env('DB_URL') ?: (env('MYSQL_URL') ?: (env('MYSQL_PRIVATE_URL') ?: env('DATABASE_URL')))) {
                    $p = parse_url($url);
